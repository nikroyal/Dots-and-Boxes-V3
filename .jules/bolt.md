...
## 2025-10-25 - O(1) Prefix Map for Route Lookups
**Learning:** `findExperienceByPath` and `isImmersivePath` previously used `Array.find` and `.some` string matching inside every component render for headers/routing. This caused O(N*M) lookup latency on page loads and resizes.
**Action:** Always pre-compute static route maps (e.g. `prefixMap`) mapping `prefix -> object` and use `Map.has` and `.startsWith` iterating over map entries as a fast-path instead of `Array.find`, converting O(N*M) worst-case string lookups to primarily O(1) operations.
## 2025-10-25 - Avoid changing Array.find behavior on optimization
**Learning:** When replacing `Array.find` or `Array.some` with a fast-path map lookup, be extremely careful to preserve the exact logical behavior of the original callbacks. If the original callback had an explicit `return false` to skip specific items (e.g., `if (!experience.immersive) return false;`), the fast-path lookup must also safely fall back to the original array iteration if the fast-path hit doesn't meet those same exclusionary criteria, rather than returning the falsey value and terminating early.
**Action:** Always write unit tests or meticulously review the boolean logic of the original array iteration callback before introducing a fast-path O(1) map lookup. Ensure the fallback array search is still executed if the fast-path candidate is rejected by the original criteria.
## 2025-10-25 - Avoid O(N) loops computing derived state during render
**Learning:** Dashboard was running an O(N) loop iterating over every single achievement to compute `upNextAchievement` on every React render. Because Dashboard mounts on the root path and responds to heartbeat snapshot events, this ran frequently, causing CPU churn calculating progress against the user profile.
**Action:** When deriving complex secondary state from an array (like filtering and computing progress for achievements) during a component's render, always wrap the computation in `useMemo` with the minimal required dependencies to avoid repeating expensive iteration logic across unchanged renders.
