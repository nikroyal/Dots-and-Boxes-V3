...
## 2026-06-24 - Array.find() Performance Bottlenecks and Import Errors
**Learning:** Replacing O(N) `Array.find()` lookups with O(1) `Map` lookups is a great optimization for static arrays like `ACHIEVEMENTS`. However, when automating replacements across multiple files, naive regex replacements (like `s/\bACHIEVEMENTS\b/ACHIEVEMENTS, getAchievementById/g`) can accidentally target loop variables (`for (const a of ACHIEVEMENTS)`) instead of just import statements, leading to syntax errors and broken builds. Furthermore, introducing new utility functions requires correctly importing them in all modified files to avoid `ReferenceError`s.
**Action:** When performing automated string replacements, use precise context matching (e.g., target only lines starting with `import`). Always run the full test suite and build verification steps immediately after making global changes to catch syntax and reference errors before they become blocking issues.
