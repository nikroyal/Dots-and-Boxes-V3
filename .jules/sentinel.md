...
## 2026-06-30 - Remove hardcoded Firebase credentials
**Vulnerability:** Hardcoded Firebase credentials (apiKey, projectId, etc.) were found in `src/lib/firebase.js`.
**Learning:** Committing hardcoded API keys directly into source code exposes sensitive environment details and allows unauthorized access if the repository is leaked or public.
**Prevention:** Always use environment variables (e.g., `import.meta.env.VITE_FIREBASE_API_KEY`) and fallback strings for missing configurations instead of pasting live secrets into source files.
