...
## 2025-02-21 - Fix missing protection for core fields in Matches rules
**Vulnerability:** The `isPlayer()` and `isHostJoining()` functions in `firestore.rules` allowed players to update match documents without restricting modifications to core fields. A malicious player could overwrite the `players` array to kick out opponents, change `gameType`, alter board dimensions (`rows`, `cols`), or prematurely mark a game as `adminClosed`. Additionally, `isHostJoining()` allowed arbitrary array modification of `players` rather than enforcing appending behavior.
**Learning:** Even when restricting document updates to authorized users (players), allowing unrestrained updates allows attackers to bypass application logic and tamper with critical match metadata.
**Prevention:** Always restrict update payloads using `changedKeys().hasAny()` or explicitly verifying additions to array fields to ensure immutable core structural fields cannot be modified through regular update flows.
