...
## 2024-06-22 - Remove Hardcoded Firebase API Key
**Vulnerability:** Found hardcoded Firebase API keys and secrets in src/lib/firebase.js.
**Learning:** Secrets should never be hardcoded into frontend configuration files as they are exposed to the public. They should be loaded dynamically via environment variables.
**Prevention:** Ensure new configurations use `import.meta.env.*` and `.env` files for secrets management instead of direct inclusion.
