...
## 2025-02-21 - Fix missing protection for core fields in Club Messages and Invites rules
**Vulnerability:** In `firestore.rules`, the `match /invites/{inviteId}` and `match /messages/{msgId}` (under clubs) rules allowed authorized users to update documents without restricting which fields could be updated. For club messages, it also lacked validation ensuring the creator's ID matched the message payload.
**Learning:** Even when restricting document creation/updates to authorized users, the allowed updates must be strictly bounded and validated. Allowing unrestrained updates on a document lets attackers bypass application logic (like altering invite core data) or impersonate other users in club chats.
**Prevention:** Always restrict update payloads using `!changedKeys().hasAny()` to ensure immutable core fields cannot be modified, and ensure creator IDs are validated against `request.auth.uid`.
