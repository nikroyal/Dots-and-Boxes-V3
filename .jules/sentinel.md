...
## 2024-10-25 - IDOR in Match Invites and Chat Append Logic
**Vulnerability:** The Firestore update rules for `invites` lacked restrictions on `changedKeys()`, allowing users to maliciously alter immutable core fields like `fromId`, `toId`, `game`, and `ts`. The rules for updating `matches` also permitted active players to bypass strictly constrained array append rules (`specJoin()`, `chatAppend()`) because the `validPlayerUpdate()` function did not exclude `spectators` and `chat`.
**Learning:** Checking for update authorization (`isSignedIn()` and matching user IDs) is insufficient to prevent manipulation of unmodifiable document fields or bypass of complex operations on related fields (such as array manipulations).
**Prevention:** Always restrict update payloads on immutable core fields via `changedKeys().hasAny()` or `!changedKeys().hasAny()` to prevent IDOR manipulation and bypasses. Ensure any fields covered by specific mutation functions (like `chatAppend()`) are correctly explicitly denied in general update operations.
