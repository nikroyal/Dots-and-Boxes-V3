...
## 2025-02-27 - Remove Hardcoded Firebase Credentials
**Vulnerability:** Hardcoded Firebase `apiKey` and other configuration values were present in `src/lib/firebase.js`, exposing sensitive credentials in the source code.
**Learning:** It is easy to accidentally commit configuration objects provided by third-party services directly into the codebase instead of using environment variables.
**Prevention:** Always extract credentials and configuration to environment variables (e.g., `import.meta.env.*` in Vite) and use a `.env.example` file to document required keys without exposing actual secrets.
