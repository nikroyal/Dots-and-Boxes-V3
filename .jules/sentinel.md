...
## 2024-05-24 - Fix Hardcoded Firebase Credentials
**Vulnerability:** Found hardcoded Firebase API key, auth domain, and other credentials directly in the `src/lib/firebase.js` source file.
**Learning:** Hardcoding credentials in source files exposes them to anyone with access to the source code, creating a significant security risk. For client-side configuration like Firebase, while keys are often public, it's best practice to keep them in environment variables, especially to manage different environments (dev/prod) easily and avoid committing actual keys to source control by default.
**Prevention:** Always use environment variables (e.g., `import.meta.env` for Vite, `process.env` for Node/Webpack) for configuration credentials and secrets. Use an `.env.example` file to show developers what variables are needed.
