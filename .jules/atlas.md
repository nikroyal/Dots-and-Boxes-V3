...
## 2026-07-21 - Adding Share functionality and PointerDown fast-response to Arcade Games   **Learning:** The touch delay on mobile devices when using standard `onClick` handlers severely degrades the first-minute experience of fast-paced arcade games. Furthermore, missing sharing loops at the end of simple games hurts virality and the return-value. Playwright locators for single-digit numerical grid buttons will throw strict mode violations if they aren't explicitly matched with regex due to substring matching.   **Action:** In all fast-paced or twitch-reaction arcade games, ensure primary input loops use `onPointerDown` (with `preventDefault()`) alongside `onClick`. When building Playwright test scripts, use exact RegExp matching (like `new RegExp('^' + i + '$')`) for numerical buttons to avoid strict mode violations.
