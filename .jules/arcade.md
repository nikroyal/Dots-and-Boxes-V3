...
## 2023-10-25 - React Timer States   **Learning:** Placing side effects like `clearInterval` inside state updater functions directly violates memory constraints due to React Strict Mode double-invocation.   **Action:** Extract timer end side-effects into a separate `useEffect` hook that watches for the end condition (e.g. `timeLeft === 0`).
## 2023-10-25 - Arcade Final Screen   **Learning:** Arcade games need to explicitly include descriptive skill tiers, share functionality, and properly close the learning loop (like showing missed answers) to satisfy constraints.   **Action:** Always include a tier scoring scale and share button on the finished state, and make sure final game state is clearly summarized for learning loop.
