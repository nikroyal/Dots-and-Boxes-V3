...
export default function GomokuHub() {
      ...
      snap.docChanges().forEach(change => {
        if (change.type === 'added' || change.type === 'modified') {
          ...
          if (inv.matchId) {
            ...
            navigate(`/gomoku/match/${inv.matchId}`);
          }
        }
      });
    });
  ...
  const handleInvite = async (e) => {
    ...
    setSending(true);
    try {
      await sendInvite(profile, target.trim(), rows, cols, 'gomoku');
  ...
  const handleQuickMatch = async () => {
    ...
    try {
      const res = await quickMatch(profile, rows, cols, 'gomoku');
