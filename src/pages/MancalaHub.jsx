...
export default function MancalaHub() {
      ...
      snap.docChanges().forEach(change => {
        if (change.type === 'added' || change.type === 'modified') {
          ...
          if (inv.matchId) {
            ...
            navigate(`/mancala/match/${inv.matchId}`);
          }
        }
      });
    });
  ...
  const handleInvite = async (e) => {
    ...
    setSending(true);
    try {
      await sendInvite(profile, target.trim(), rows, cols, 'mancala');
  ...
  const handleQuickMatch = async () => {
    ...
    try {
      const res = await quickMatch(profile, rows, cols, 'mancala');
