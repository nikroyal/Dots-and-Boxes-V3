...
export default function OthelloHub() {
      ...
      snap.docChanges().forEach(change => {
        if (change.type === 'added' || change.type === 'modified') {
          ...
          if (inv.matchId) {
            ...
            navigate(`/othello/match/${inv.matchId}`);
          }
        }
      });
    });
  ...
  const handleInvite = async (e) => {
    ...
    setSending(true);
    try {
      await sendInvite(profile, target.trim(), rows, cols, 'othello');
  ...
  const handleQuickMatch = async () => {
    ...
    try {
      const res = await quickMatch(profile, rows, cols, 'othello');
