...
export default function AxiomHub() {
  ...
  const profile = {
    ...
    dailyGoalStreak: 5,
    dailyGoalDate: getLocalYYYYMMDD(), // This makes it completed
    dailyStats: {
        date: getLocalYYYYMMDD(), wins: 1, gamesPlayed: 1, totalBoxes: 5, biggestChain: 2
    }
  };
  ...
  const upNextAchievement = useMemo(() => getUpNextAchievement(profile, true, true), [profile]);
          ...
          {dailyGoal && (
            <Link to="/dots-and-boxes" className="block border hairline p-4 bg-black/5 hover:bg-black/10 transition-colors" style={{ borderColor: dailyGoalCompleted ? 'var(--forest)' : 'var(--hairline)' }}>
              <div className="font-mono text-[0.55rem] tracking-widest uppercase mb-1 flex items-center gap-2 opacity-60">
                ...
                {dailyGoalCompleted && (profile.dailyGoalStreak || 0) > 0 && (
