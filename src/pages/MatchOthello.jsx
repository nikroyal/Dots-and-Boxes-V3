...
import { applyMove } from '../lib/othelloLogic';
...
export default function MatchOthello() {
  const { id } = useParams();
  ...
  const handleMove = async (r, c) => {
    ...
    if (busy === 'move') return;
    setBusy('move');
    try { await makeMove(id, "othello", null, r, c, profile); }
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-center">
          {concealBoard ? (
            ...
            <Board game={match.game} players={match.players} playerInfo={match.playerInfo} isMyTurn={isMyTurn} onPlay={handleMove} lastMove={lastMove} finished={match.status === 'finished'} />
          )}
        </div>
