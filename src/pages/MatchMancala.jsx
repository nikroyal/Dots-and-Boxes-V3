...
import { applyMove } from '../lib/mancalaLogic';
...
export default function MatchMancala() {
  const { id } = useParams();
  ...
  const handleMove = async (r, c) => {
    ...
    if (busy === 'move') return;
    setBusy('move');
    try { await makeMove(id, "mancala", null, r, c, profile); }
        ...
        <div className="flex justify-center">
          {concealBoard ? (
            ...
            <Board game={match.game} players={match.players} playerInfo={match.playerInfo}
                   isMyTurn={isMyTurn}  lastMove={lastMove} />
          )}
        </div>
...
function Board({ game, players, playerInfo, isMyTurn, handleMove, lastMove, myIdx }) {
