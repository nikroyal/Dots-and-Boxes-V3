...
export default function RpsHub() {
      ...
      snap.docChanges().forEach(change => {
        if (change.type === 'added' || change.type === 'modified') {
          ...
          if (inv.matchId) {
            ...
            navigate(`/rps/match/${inv.matchId}`);
          }
        }
      });
    });
  ...
  const handleInvite = async (e) => {
    ...
    setSending(true);
    try {
      await sendInvite(profile, target.trim(), rows, cols, 'rps');
  ...
  const handleQuickMatch = async () => {
    ...
    try {
      const res = await quickMatch(profile, rows, cols, 'rps');
