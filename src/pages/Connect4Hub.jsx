...
export default function Connect4Hub() {
      ...
      snap.docChanges().forEach(change => {
        if (change.type === 'added' || change.type === 'modified') {
          ...
          if (inv.matchId) {
            ...
            navigate(`/connect4/match/${inv.matchId}`);
          }
        }
      });
    });
  ...
  const handleInvite = async (e) => {
    ...
    setSending(true);
    try {
      await sendInvite(profile, target.trim(), rows, cols, 'connect4');
  ...
  const handleQuickMatch = async () => {
    ...
    try {
      const res = await quickMatch(profile, rows, cols, 'connect4');
