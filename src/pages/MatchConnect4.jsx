...
export default function MatchConnect4() {
  const { id } = useParams();
  ...
  const handleMove = async (c) => {
    if (!isMyTurn) return;
    if (busy === 'move') return;
    setBusy('move');
    try { await makeMove(id, "connect4", null, null, c, profile); }
    ...
    <div className="border hairline px-4 py-2 flex items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-3">
        <div className="hidden sm:block" style={{ width: 80, height: 4, background: 'var(--hairline)' }}>
          <div role="progressbar" aria-valuenow={seconds} aria-valuemin={0} aria-valuemax={timeoutMs / 1000} style={{ width: `${fraction * 100}%`, height: '100%', background: color, transition: 'width 1000ms linear' }} />
        </div>
